...
def check_rel_convergence(check_timestep, E_threshold=1e-3, P_threshold=1e-3,
    ...
    print("{:^47}".format("Convergence Check"))
    print("{:<5}    {:^11}   {:^11}   {:^11}".format(
    ...
    for i in range(num_steps - 1):
        ...
        for val0, val, threshold in zip(val_list0, val_list, thresholds):
            ...
            print("{:>14.4E}".format(conv_val), end="")
    ...
    print("-" * 47)
    if converged_teq:
        print("{:^47}".format(f"Converged at the time %.2f a.u." % teq))
        print("-" * 47)
        print("Warmup stage ended according to set thresholds:")
        print("{:^16}{:^15}{:^16}".format("Rel. DE", "Rel. DP", "Rel. DT"))
        print("{:^16.1E}{:^15.1E}{:^16.1E}".format(thresholds[0], thresholds[1], thresholds[2]))
        print("-" * 47)
        print("Averaged values after this time:")
        print("System energy      E_tot :{:>21.4f}".format(converged_avgs[0]))
        print("System pressure    P     :{:>21.4f}".format(converged_avgs[1]))
        print("System temperature T     :{:>21.4E}".format(converged_avgs[2]))
        return converged_teq
    else:
        print("{:^47}".format(f" Did not converge!"))
        return None
...
if __name__ == '__main__':
    ...
    plot_eq_RDF(f"{args.file}_plot_4.png", warmup=x_warmup)
