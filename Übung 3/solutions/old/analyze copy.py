...
def plot_data(plot_name, M=None, warmup=None):
    ...
    ax_2.plot(avg_rdfs[0, :], avg_rdfs[1, :])
    ax_2.set_xlabel(r"distance $r$ in a.u.")
    ax_2.set_ylabel(r"RDF, $g(r)$ in a.u.")
    ax_2.set_title("Radial Destribution Function")

    # ------- Plot for Pressure -------
    ax_3 = fig.add_subplot(grid[1, 0])
    ax_3.plot(time_range_P, pressures)
    ax_3.set_ylabel(y_label + "pressure in a.u.")
    ax_3.set_title("Pressure")
    ax_3.set_xlim(min_x_P, max_x_P)
    ax_3.set_ylim(min_P - delta_P * b_offst, max_P + delta_P * b_offst)

    # ------ Plot for Temperature -----
    ax_4 = fig.add_subplot(grid[1, 1])
    ax_4.plot(time_range_T, temperatures)
    ax_4.set_ylabel(y_label + "temperature in a.u.")
    ax_4.set_title("Temperature")
    ax_4.set_xlim(min_x_T, max_x_T)
    ax_4.set_ylim(min_T - delta_T * b_offst, max_T + delta_T * b_offst)
    ...
    ax_3.axhline(y=avg_pressure, color='red', alpha=0.3, linestyle='-')
    ax_4.axhline(y=avg_temperature, color='red', alpha=0.3, linestyle='-')

    fig.set_dpi(100)
    plt.tight_layout()
    plt.savefig(plot_path/plot_name)

def plot_RDF(plot_name, warmup=None):
    # Averaging of RDF
    start_index = int(np.ceil(warmup/DT)) if warmup > 0 else 0
    ...
    plt.set_xlabel(r"distance $r$ in a.u.")
    plt.set_ylabel(r"RDF, $g(r)$ in a.u.")
    plt.set_title("Radial Destribution Function")
