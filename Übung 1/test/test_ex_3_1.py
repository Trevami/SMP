...
class Tests(ut.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.x_init = data['x_init'].transpose()
        cls.v_init = data['v_init'].transpose()
        cls.masses = data['m']
        cls.g = data['g']
    ...
    def test_step_euler_force(self):
        ...
        x_new, v_new = ex_3_1.step_euler(
            x, v, dt, self.masses, self.g, ex_3_1.forces(self.x_init, self.masses, self.g))
        ...
        F_mat = ex_3_1.forces(self.x_init, self.masses, self.g)
        forces = []
        for i in range(np.shape(F_mat)[1]):
            forces.append([np.sum(F_mat[0, i, :]), np.sum(F_mat[1, i, :])])
        np.testing.assert_almost_equal(
            forces,
